...
# that search, not from guesswork: it is what removes supermarkets from florists and a bus stop from crematoria.
def _load_trades():
    here = os.path.dirname(os.path.abspath(__file__))
    with open(os.path.join(here, "..", "trades.json"), encoding="utf-8") as f:
        d = json.load(f)
    return {k: (v["terms"], v["keep"], v["drop"]) for k, v in d.items()}
