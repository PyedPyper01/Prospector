...
6. description — ONLY when verdict is APPROVE. 1-2 sentences, warm British English, that capture what makes THIS firm DIFFERENT from others in the same category — the single distinctive thing a family would use to tell it apart, LED WITH FIRST. Mine the firm's own site text for the differentiator, e.g.: how long-established / founding year ("the town's longest-established independent adviser"); a named specialism ("inheritance-tax and trustee specialists — the one to look at for an estate"); a distinctive model or fee stance ("salaried advisers, no commission"); a concrete facility or provision (funeral director: "family-run since 1923, own chapel of rest, faith provision"; "own private ambulance, out-of-hours service").
   HARD RULES: (a) Lead with the differentiator, not a generic opener. (b) BAN generic filler that is true of nearly every firm in the category — "independently owned", "family-run" ALONE, "FCA-regulated", "friendly/professional service", "trusted", "here to help": these are NOT differentiators and must NOT be the description (regulation/accreditation belong in the credentials array). (c) Use ONLY facts on the firm's own site — NEVER invent a specialism, founding date, facility, faith provision or claim; if two firms' sites are similar, find the smaller real distinction rather than inventing one. (d) If the site genuinely shows nothing distinctive, write one honest plain sentence of what they actually do — do not manufacture a differentiator. (e) No sales language, no "best"/"leading". (f) Keep credentials OUT of the description — they live in the separate credentials array. If verdict is not APPROVE, description="".

Respond with ONLY a JSON array, no markdown, no preamble, one object per input firm IN ORDER:
[{"i":0,"independent":true,"group_quote":"","fit":8,"quality":7,"verdict":"APPROVE","reason":"independent estate-planning IFA, STEP, strong probate offering","credentials":"Chartered, STEP, est. 1974","description":"Estate and inheritance-tax specialists — the practice to look at for probate and trustee work, with in-house STEP practitioners. Established in the town since 1974."}]
reason: under 18 words. The description must LEAD with the differentiator (here: estate/IHT/trustee specialism + longevity), never a generic opener. Be decisive and strict — a wrong lead costs the family's trust.`;
  ...
  try {
    ...
    const results = items.map((it, i) => {
      ...
      if (v) {
        ...
        base.description = String(v.description || "").slice(0, 1000);
      } else if (apiKey) {
